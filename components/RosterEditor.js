...
export default function RosterEditor() {
  ...
  const [rosterType, setRosterType] = useState("combined");
  ...
  function autoFill() {
    ...
    setEntries((prev) => {
      ...
      days.forEach((d, i) => {
        ...
        const value = TIMED_ROWS.has(autoFillRow) ? `${staffName} (${defaultTime})` : staffName;
  ...
  async function exportPng() {
    ...
    try {
      await exportNodeAsPng(tableRef.current, `${fileTag(rosterType)}-roster-${startDate}.png`);
  ...
  async function downloadPreview() {
    await exportNodeAsPng(
      previewRef.current,
      `${fileTag(previewRoster.roster_type)}-roster-${previewRoster.start_date}.png`
    );
  }
  ...
  function onDragEnd(e) {
    ...
    if (cell && dragRef.current) {
      ...
      if (payload.type === "staff") {
        const value = TIMED_ROWS.has(targetRow)
  ...
  function renderEditableRow(label, toneClass) {
    return (
                ...
                {list.map((val, idx) => (
                  <div className="cell-chip" key={idx}>
                    <span
                      ...
                      title="Drag to move"
                    >
                      <span className="avatar-dot avatar-dot-sm" style={{ background: avatarColor(val) }}>
                        {initials(val)}
                      </span>
                      {val}
                    </span>
            ...
            <div className="controls-row" style={{ marginBottom: 14 }}>
              ...
              <div className="field">
                ...
                <select value={archiveFilter} onChange={(e) => setArchiveFilter(e.target.value)}>
                  ...
                  <option value="combined">Weekly Roster</option>
                  <option value="shift">Extend Roster (legacy)</option>
                  <option value="dedicated">Evening Roster (legacy)</option>
                </select>
              </div>
              {role !== "staff" && (
            ...
            <div className="tag-toggle">
              {Object.entries(TYPE_META).map(([key, m]) => (
                <button key={key} className={rosterType === key ? "active" : ""} onClick={() => setRosterType(key)}>
                  {m.label}
                  {key === "combined" && <span className="tag-badge">Both, one image</span>}
                </button>
              ))}
            </div>
            {rosterType === "combined" && (
              <p className="hint combined-hint">
                One board for both duties — drag staff onto the Extend row or any Evening row below, save once,
                and download a single image with both rosters clearly separated by colour.
              </p>
            )}
            ...
            <section className="panel">
              ...
              <div className="staff-chip-list">
                {staffList.map((s) => (
                  ...
                  >
                    <span className="avatar-dot" style={{ background: avatarColor(s.name) }}>
                      {initials(s.name)}
                    </span>
                    {s.name}
                  </div>
                ...
                <table className="roster-table">
                  ...
                  <tbody>
                    {meta.sections
                      ...
                      : rowLabels.map((label) => renderEditableRow(label))}
                  </tbody>
                </table>
              </div>
            </div>
            ...
            <div className="export-wrap" style={{ margin: "16px 0" }}>
              <div className="export-inner" ref={previewRef}>
                ...
                <table className="roster-table">
                  ...
                  <tbody>
                    {(() => {
                      const pMeta = TYPE_META[previewRoster.roster_type];
                      const pDays = generateWeek(previewRoster.start_date);
                      ...
                      if (pMeta?.sections) {
                        return pMeta.sections.map((section) => (
                          <Fragment key={section.key}>
                            <tr className={`section-row section-${section.tone}`}>
                      ...
                      return (previewRoster.row_labels || pMeta?.rows || []).map((label) => renderRow(label));
                    })()}
                  </tbody>
                </table>
              </div>
